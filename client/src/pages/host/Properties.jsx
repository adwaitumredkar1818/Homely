  ...
  return (
    <div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-12 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-surface p-8 rounded-[2.5rem] border border-border shadow-xl flex items-center gap-6 group hover:border-primary/20 transition-all">
            <div className="w-16 h-16 bg-green-500/10 rounded-3xl flex items-center justify-center text-green-600 group-hover:scale-110 transition-transform">
         ...
         <div className="bg-surface p-8 rounded-[2.5rem] border border-border shadow-xl flex items-center gap-6 group hover:border-primary/20 transition-all">
            <div className="w-16 h-16 bg-blue-500/10 rounded-3xl flex items-center justify-center text-blue-600 group-hover:scale-110 transition-transform">
         ...
         <div className="bg-surface p-8 rounded-[2.5rem] border border-border shadow-xl flex items-center gap-6 group hover:border-primary/20 transition-all">
            <div className="w-16 h-16 bg-purple-500/10 rounded-3xl flex items-center justify-center text-purple-600 group-hover:scale-110 transition-transform">
      ...
      <div className="bg-surface p-10 rounded-[3rem] border border-border shadow-2xl space-y-8 animate-in fade-in zoom-in duration-1000">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-surface/50 p-4 rounded-[2rem] border border-border">
         <div className="flex bg-background p-1 rounded-2xl w-full md:w-auto">
            {['ALL', 'BOOKED', 'FREE'].map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`flex-1 md:flex-none px-8 py-2.5 rounded-xl text-xs font-black tracking-widest transition-all ${filter === f ? 'bg-primary text-background shadow-lg' : 'text-taupe hover:text-primary'}`}
              >
                {f}
              </button>
            ))}
         ...
         <div className="relative w-full md:w-80">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-3 bg-surface border border-border rounded-2xl text-sm font-bold focus:ring-4 focus:ring-primary/5 outline-none transition-all"
            />
         </div>
      </div>

      {/* Property Table */}
      <div className="bg-surface rounded-[3rem] border border-border shadow-2xl overflow-hidden">
        {filteredProperties.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-background/30 border-b border-border text-[10px] font-black text-taupe uppercase tracking-[0.2em]">
              ...
              <tbody className="divide-y divide-border">
                {filteredProperties.map((room) => (
                  <tr key={room.id} className="hover:bg-background/10 transition-all group animate-in fade-in duration-500">
                    <td className="p-8 pl-10 flex items-center gap-6">
                      <div className="w-24 h-16 rounded-2xl overflow-hidden shrink-0 border border-border shadow-sm relative">
                        <img 
          ...
          <div className="text-center py-24 bg-background/10">
             <div className="w-24 h-24 bg-surface rounded-full flex items-center justify-center mx-auto mb-8 shadow-xl border border-border">
                <Building2 className="w-12 h-12 text-taupe/40" />
             </div>
