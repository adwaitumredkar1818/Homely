        ...
        <div className="flex flex-wrap items-center gap-4">
             <div className="relative flex-1 md:w-64">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-surface border border-border rounded-xl focus:ring-2 focus:ring-accent/50 outline-none transition-all text-sm font-medium text-primary placeholder-taupe/40"
                />
             </div>
             ...
             <div className="flex bg-surface p-1 rounded-xl border border-border shadow-sm">
               {['ALL', 'PENDING', 'CONFIRMED', 'CANCELLED'].map((f) => (
                 <button
                   key={f}
                   onClick={() => setFilter(f)}
                   className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${filter === f ? 'bg-primary text-background shadow-md' : 'text-taupe hover:text-primary'}`}
                 >
      ...
      {filteredReservations.length > 0 ? (
        <div className="grid grid-cols-1 gap-6">
           {filteredReservations.map((res) => (
             <div 
                 key={`${res.id}-${res.type}`}
                 className="bg-surface rounded-[2rem] border border-border p-6 flex flex-col lg:flex-row gap-8 hover:shadow-xl transition-all group"
              >
                {/* Property Preview */}
                <div className="w-full lg:w-48 h-32 rounded-2xl bg-background overflow-hidden shrink-0 relative">
                   <img 
                    ...
                    {res.status === 'PENDING' ? (
                      <>
                          <button 
                            ...
                            className="flex-1 flex items-center justify-center gap-2 bg-primary text-background py-3 rounded-xl font-bold hover:bg-accent hover:text-background transition-all shadow-lg shadow-blue-500/10"
                         >
                           <CheckCircle2 className="w-4 h-4" /> Approve
                        </button>
                        <button 
                           onClick={() => handleStatusUpdate(res.id, res.type, 'CANCELLED')}
                           className="flex-1 flex items-center justify-center gap-2 border border-red-500/20 text-red-500 py-3 rounded-xl font-bold hover:bg-red-500/10 transition-all"
                        >
                      ...
                      <button 
                        ...
                        className="w-full flex items-center justify-center gap-2 bg-background text-primary py-3 rounded-xl font-bold hover:bg-white/5 transition-all"
                      >
          ...
          <div className="text-center py-20 bg-surface rounded-[3rem] border border-dashed border-border">
             <Calendar className="w-16 h-16 text-taupe/40 mx-auto mb-6" />
